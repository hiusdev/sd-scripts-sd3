...
def analyze_state_dict_state(state_dict: Dict, prefix: str = ""):
    print(f"Analyzing state dict state...")
    ...
    print(f"Analyzed state dict state: {params}")
    return params
...
def load_clip_l(
    ...
    if clip_l_path is None:
        if "text_encoders.clip_l.transformer.text_model.embeddings.position_embedding.weight" in state_dict:
            ...
            print("clip_l is not included in the checkpoint and clip_l_path is not provided")
            return None
    ...
    if clip_l_sd is None:
        print(f"Loading state dict from {clip_l_path}")
    ...
    if "text_projection.weight" not in clip_l_sd:
        print("Adding text_projection.weight to clip_l_sd")
...
def load_clip_g(
    ...
    if state_dict is not None:
        if "text_encoders.clip_g.transformer.text_model.embeddings.position_embedding.weight" in state_dict:
            ...
            print("clip_g is not included in the checkpoint and clip_g_path is not provided")
            return None

    # load clip_g
    print("Building CLIP-G")
    config = CLIPTextConfig(
        vocab_size=49408,
        hidden_size=1280,
    ...
    if clip_g_sd is None:
        print(f"Loading state dict from {clip_g_path}")
...
def load_t5xxl(
    ...
    if state_dict is not None:
        if "text_encoders.t5xxl.transformer.encoder.block.0.layer.0.SelfAttention.k.weight" in state_dict:
            ...
            print("t5xxl is not included in the checkpoint and t5xxl_path is not provided")
            return None
